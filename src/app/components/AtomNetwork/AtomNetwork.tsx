...
const Network: React.FC<NetworkProps> = ({filteredCurSnap}) => {
  ...
  if (filteredCurSnap) {
    atomAndSelectorArr = Object.entries(filteredCurSnap);
  }
  ...
  const [atomList] = useState(
    atomAndSelectorArr.filter(([isAtom, obj]) =>
      !obj.nodeDeps.length ? isAtom : null,
    ),
  );
  // array of selectors (what the drop down showSelectorMenu is going to display)
  const [selectorList] = useState(
    atomAndSelectorArr.filter(([isSelector, obj]) =>
      obj.nodeDeps.length ? isSelector : null,
    ),
  );
                  ...
                  <p
                    ...
                    onClick={(e: React.MouseEvent) => {
                      ...
                      document
                        ...
                        .setAttribute('style', 'opacity: 100%;');
                      document
                        .querySelectorAll('.SelectorListItem')
                        .forEach(item => {
                          if (item.id !== `Selector${i}`)
                            item.setAttribute('style', 'opacity: 30%;');
                        });
                      //set the search value to the name of the paragraph element to render only corresponding and related nodes
                      setSearchValue((e.target as Element).innerHTML);
